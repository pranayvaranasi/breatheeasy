...
"""
Configuration loader for the AetherIQ project.

Loads configuration from config/config.yaml and makes it available as the CONFIG dictionary.
"""
...
# --- Import Health Risk Interpreter ---
...
def load_config():
    ...
    Raises:
        ...
        yaml.YAMLError: If config.yaml cannot be parsed.
    """
...
try:
    ...
    log.error(f"Critical: Could not load configuration. Using defaults. Error: {e}")
    CONFIG = {
        'apis': {
            'aqicn': {'base_url': "https://api.waqi.info/feed"},
            'weatherapi': {'base_url': "http://api.weatherapi.com/v1/current.json"}
        },
        'modeling': {
            'target_cities': ['Bangalore', 'Chennai', 'Hyderabad', 'Mumbai'],
            'forecast_days': 3,
            'weather_regressors': [
                'temperature_2m', 'relative_humidity_2m', 'precipitation',
                'pressure_msl', 'cloud_cover', 'wind_speed_10m', 'dew_point_2m'
            ]
        },
        'api_timeout_seconds': 10,
        'api_retries': {'default': 2},
        'api_retry_delay_seconds': {'default': 1},
        'logging': {'level': 'INFO'}
    }
